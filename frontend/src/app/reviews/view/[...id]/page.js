...
const Page = () => {
  ...
  const [addToBankDialogOpen, setAddToBankDialogOpen] = useState(false);
  const { data: userData } = useSWR({ url: "/Authentication/UserInfo" });
  ...
  const handleAddToBank = (decision) => {
    setAddToBankDialogOpen(true);
    setAddToBank(decision);
  };
  ...
  return !data ? (
    ...
    <div className="w-full flex justify-between gap-4">
      <Card className="flex flex-col gap-0.5 w-1/2 p-4 bg-gray-50">
        ...
        {reviewerWithLog.find((o) => o.appUserId === userData.id)?.log ===
          null && (
        ...
        {data.isAddedToQuestionBank == null && _.includes(roles, "Admin") && (
          <div className="flex gap-2 mt-5">
            <Button
              variant="contained"
              color="primary"
              onClick={() => handleAddToBank(true)}
            >
              Add to QuestionBank
            </Button>
            <Button
      ...
      />
      <AddToBankConfirmation
        {...{
          open: addToBankDialogOpen,
          approved: addToBank,
          mutate,
          handleClose: () => {
            setAddToBankDialogOpen(false);
          },
        }}
      />
    </div>
